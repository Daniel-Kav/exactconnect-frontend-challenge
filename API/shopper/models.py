...
class Wishlist(models.Model):
    ...
    product_id = models.IntegerField()  # Store the product ID from FakeStore API
    title = models.CharField(max_length=255)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    class Meta:
        unique_together = ('user', 'product_id')  # Prevent duplicate wishlist items
        
    def __str__(self):
        return f"{self.user.email}'s wishlist item: {self.title}"
